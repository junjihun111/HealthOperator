...
<body>
	...
	<a href="/health_member_manager/health/endedShoeNoEndNoListPage.do?searchNo=${sessionScope.searchNo }"><button>만료된 신발장 조회</button></a>
	...
	<table border='1' id="HealthMemberTB">
		...
		<tbody id="listTbody">
				...
				<tr>
					<td>${requestScope.healthMemberNoList.memberNo}</td>
					<td>${requestScope.healthMemberNoList.memberName}</td>
					<td>${requestScope.healthMemberNoList.phoneEnd}</td>
					<td><fmt:formatDate value="${requestScope.healthMemberNoList.startDay}" type="date" pattern="yyyy년 MM월 dd일 " /></td>
					<td><fmt:formatDate value="${requestScope.healthMemberNoList.endDay}" type="date" pattern="yyyy년 MM월 dd일" /></td>
					<td><a href="/health_member_manager/health/shoescageBymemberNo.do?memberNo=${requestScope.healthMemberNoList.memberNo}&memberName=${requestScope.healthMemberNoList.memberName}">${requestScope.healthMemberNoList.shoesNo}</a></td>
					<td><a href="/health_member_manager/health/attendanceByNo.do?memberName=${requestScope.healthMemberNoList.memberName }&memberNo=${requestScope.healthMemberNoList.memberNo}">출석하기</a>
						/ <a href='/health_member_manager/health/attendanceListByNoList.do?memberNo=${requestScope.healthMemberNoList.memberNo}'>출석내역보기</a></td>
		</tbody>
	</table>
	...
	<div id="dialog" title="회원 정보">
		<form id="test" method="post"
			...
			<table border=1>
				<tr><td>회원번호</td><td id="memberNo1"></td></tr>
				<tr><td>회원이름</td><td id="memberName1"></td></tr>
				<tr><td>핸드폰번호</td><td><span id="phoneNumber1"></span>- <span id="phoneNumber2"></span>-<span id="phoneNumber3"></span></td></tr>
				<tr><td>시작일</td><td id="startDay1"></td></tr>
				<tr><td>만료일</td><td id="endDay1"></td></tr>
				<tr><td>신발장번호</td><td id="shoesNo1"></td></tr>
				<tr><td>생년월일</td><td id="birthday1"></td></tr>
				<tr><td>주소</td><td id="address1"></td></tr>
				<tr><td>이메일</td><td id=email1></td></tr>
				<tr><td>환불금액</td><td id="returnPrice1"></td></tr>
			</table>
		</form>
	...
	<div id=updateMemberPage title="회원 수정 " style="display: none;">
		<form action="/health_member_manager/health/updateTransferByNo.do" method="post" name="update" onsubmit="return updateFormCheck()">
	...
	<div id="shoescageInfo" title="신발장 정보" style="display: none">
		...
		<a href="/health_member_manager/health/ShoesCageStopByNo.do?memberNo=${requestScope.shoescage.memberNo}"><button>이용중지</button></a>
	</div>
	...
	<div id="shoesCageRegisterPage" title="신발장등록" style="display: none">
		...
		<br>
		<form action="/health_member_manager/health/shoesRegisterByNo.do" method="get" class="f_register" name="shoesCage" onsubmit="return shoesCageCheck()">
...
<div id="endedShoeNoDialog" title="만료된 신발장 조회" style="display: none">
		<form action="/health_member_manager/health/deleteEndedShoesCageForNoPage.do">
		<input type="hidden" name="searchNo" value="${sessionScope.searchNo }">
